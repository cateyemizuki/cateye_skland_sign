...
> **说明**：森空岛 API 不提供独立的「今日是否已签到」查询接口，因此
> `/森空岛状态`、`skland_sign_status` 在查询时会执行一次签到流程，再根据返回
> 结果判断是否已签到（已签到的账号会返回"今日已签到"信息，不会重复发放奖励）。
> 即**每次查询状态等于一次签到尝试**，这是 API 设计限制下的取巧做法，属正常行为。
